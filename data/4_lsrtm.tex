...
1、参数耦合不强烈时：采用解耦预条件的梯度进行$\delta V_p$与$\delta V_s$同时反演；

2、参数耦合强烈时：先采用预条件的梯度对$\delta V_s$进行反演，在$\delta V_s$反演足够好的时候对$\delta V_p$进行反演。
...
V_s$的反演就会受到参数耦合的干扰，而$V_p$中则较少受到$V_s$结构的影响，那么我们可以采用第一中反演策略，同时反演$\delta
