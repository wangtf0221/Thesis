...
\begin{equation}
        ...
        \label{eq:L_rho}
\end{equation}
在ELSRTM中通常假设背景波场及背景模型不发生变化。%故可将方程\eqref{eq:WE_Matrix_delta}看作为新的状态方程。
...
也就是说，如果模型中包含较强的反射能量时就需要匹配剩余反射数据，而不是反射数据。
...
\begin{figure}[!htb]
   ...
   \caption{常规ERTM与ELSRTM结果。左侧为$\delta V_p$，右侧为$\delta V_s$。(a), (b)为ERTM结果；(c),
   (d)为ELSRTM结果。}
...
\begin{figure}[!htb]
   ...
   (b)为ERTM结果; (c), (d)为采用策略1时ELSRTM结果；(e), (f)为采用策略2时ELSRTM结果。}
...
\begin{figure}[!htb]
   ...
   \caption{Born模拟数据常规ERTM与ELSRTM结果。左侧为$V_p$，右侧为$V_s$。第一行为ERTM结果，第二行为ELSRTM结果。}
...
在引入模式解耦之后(图\ref{fig:de_1_refl})，可以看到ERTM结果中$\delta
